    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["UserInfo"] != null)
            {
                ...
                //等级
                var rankInfo = TCJJG.Web.UserCenter.UserCenter.UserInfo().GetRankByExperience(userWeb.UserID);
                if (null!=rankInfo)
                {
            ...
            this.hdUserType.Value = Convert.ToString(userType);
            ...
            hdrank.Value = Convert.ToString(userRank);
        }
    }
}
