...
public partial class PayCenter_Yeepay_Callback : System.Web.UI.Page
{
            ...
            if (string.IsNullOrEmpty(result.ErrMsg))
            {
                if (result.R1_Code == "1")
                {
                    ...
                    catch (Exception e3)
                    {
                        ...
                        FFJJG.Server.Utils.Logging.write("充值","易宝回发充值信息#1.R6_Order：" + result.R6_Order + "。R3_Amt：" + result.R3_Amt + "。r2_TrxId" + R2_TrxId + ".异常代码:" + e3.Message,true);
                    }
                    finally
                    {
                ...
                {
                    FFJJG.Server.Utils.Logging.write("充值","易宝回发充值信息#2.密钥验证失败",true);
        ...
        string Rp_PayDate, string Rq_CardNo, string Ru_Trxtime)
    {
        ...
        {
            FFJJG.Server.Utils.Logging.write("充值","易宝回发充值信息#3.异常代码:opeCreateOrder：" + opeCreateOrder + "。callbackID：" + callbackID + "。R6_Order：" + R6_Order + "。R3_Amt：" + R3_Amt + "。R2_TrxId" + R2_TrxId,true);
            return -1;
        }
        //更新财富
        ...
        if (ret3 < 0)
        {
           FFJJG.Server.Utils.Logging.write("充值","易宝回发充值信息#4.异常代码:opeCreateOrder：" + opeCreateOrder + "。callbackID：" + callbackID + "。R6_Order：" + R6_Order + "。R3_Amt：" + R3_Amt + "。R2_TrxId" + R2_TrxId,true);
            return -1; ;
        }
        else
        {
    ...
    private static string valiStr(string valiStr, string flag, int strLegth, string R6_Order)
    {
        ...
        FFJJG.Server.Utils.Logging.write("充值","易宝回发充值信息#5.。非密钥参数异常。R6_Order：" + R6_Order +
            "。valiStr：" + valiStr + "。flag：" + flag + "。strLegth：" + strLegth,true);
        return "非密钥参数异常";
    }
}
