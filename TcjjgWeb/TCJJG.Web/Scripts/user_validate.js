...
//    if ($("#ctl00_ContentPlaceHolder1_txtNickName").val().length == 0) {
//        if (!confirm("你的昵称为:" + $("#ctl00_ContentPlaceHolder1_lblMarkUserName").text() + "。你确定使用吗？")) {
//            return false;
//        } else {
//            return true;
//        }
...
function pv_B_NickName_0(divNick_txtNickName, c_divNick_txtNickName) {
    ...
    $("#nickNameFinish").show();
    $("#nickNameError").hide();
    if (trim(G(divNick_txtNickName).value).length == 0) {
        $("#cnickName").show();
        $("#cnickName").text("如果不填写昵称，系统将自动使用你的用户名: " + $("#ctl00_ContentPlaceHolder1_lblMarkUserName").text() + " 作为你的昵称。");
        $("#cnickName").css("color", "#3d3d3d");
    }
    return true;
}
